 ...
 * java Homework4.IntCalculator "(2 x 3) ^ 2"
 * @author Brian S. Borowski, Rhys Talley (rt2860)
...
public class IntCalculator {
     ...
     * Homework4.Error object is set accordingly.
     * @param expression the infix expression supplied by the user
     */
    ...
    public String infixToPostfix() throws StackException {
        // TODO
        MyStack<Character> s = new MyArrayList<>(infixExpression.length());
        MyStack<Integer> integers = new MyArrayList<>(infixExpression.length());
        StringBuilder postfix = new StringBuilder();
        int str_builder_index = 0;
        int consecutiveNumbers = 0;
        for(int i=0; i<infixExpression.length(); i++){
            Character char1 = infixExpression.charAt(i);
            if(isWhiteSpace(char1)){
                ...
                if (consecutiveNumbers>0){
                    postfix.delete(str_builder_index-(1+consecutiveNumbers), str_builder_index);
                    str_builder_index -= (1+consecutiveNumbers);
                    Integer sum = integers.pop()*10 + Character.getNumericValue(char1);
                    integers.push(sum);
                    postfix.append(sum + " ");
                    str_builder_index += 2+consecutiveNumbers++;
                }
                else{
                    integers.push(Character.getNumericValue(char1));
                    postfix.append(char1);
                    postfix.append(" ");
                    str_builder_index += 2;
                    consecutiveNumbers++;
                }
            }
            else if (isParenthesis(char1)){
                consecutiveNumbers = 0;
                if(char1=='('){
                    s.push(char1);
                }
            ...
            else if(isUnaryOperator(char1)){
                consecutiveNumbers = 0;
                s.push(char1);
            }
            else if(isBinaryOperator(char1)){
                consecutiveNumbers = 0;
                while(!s.isEmpty() && precedence(char1)<=precedence(s.peek())){
                    if(!isLeftAssociative(char1) && !isLeftAssociative(s.peek())){
                        break;
                    }
                    else{
                        postfix.append(s.pop());
    ...
    public int evaluatePostfix()
        ...
        // TODO
        String postfix = postfixExpression + " ";
        MyStack<Integer> ints = new MyArrayList<>(postfix.length());
        Integer index;
        Integer sum;
        while (postfix.length()>1){
            Character char1 = postfix.charAt(0);
            index = postfix.indexOf(" ");
            if (index>1 || isDigit(char1)){
                sum = Integer.parseInt(postfix.substring(0, index));
                ints.push(sum);
            }
            else if (isUnaryOperator(char1)){
                ...
                Integer num2 = ints.pop();
                Integer num1 = ints.pop();
                switch(char1) {
                    case '+':
                        ints.push(num1+num2);
                        break;
    ...
    public static void main(String[] args) {
        ...
        if (input.length() == 0) {
            System.err.println("Usage: java Homework4.IntCalculator <expression>");
            System.exit(1);
        }
        ...
        try {
            ...
            System.err.println("Homework4.Error:              " + e.getMessage());
        }
    }
}
